...
def parse_event(event):
    ...
    try:
        ...
        try:
            ...
            # If the time part starts with a single digit hour (e.g., '7:46:01.474218')
...
# The session tuple now is (start, end, project, color).
sessions = []
...
# We use the session’s start date (as an ISO string) as the key.
sessions_by_day = {}
...
for i, day in enumerate(days):
    for start, end, project, color in sessions_by_day[day]:
        ...
        # Use the stored color (a hex string) for the rectangle.
...
def format_time(hour_float):
    ...
    if total_minutes >= 1440:
        return "12:00 AM"
    dt = datetime(2000, 1, 1, total_minutes // 60, total_minutes % 60)
...
# Manually adjust margins to prevent dynamic resizing issues.
